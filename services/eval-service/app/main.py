...
from app.routes import stats, traces
...
async def lifespan(app: FastAPI):
    log.info("eval_service_startup", service=settings.service_name)
    await init_db()
    yield
    log.info("eval_service_shutdown", service=settings.service_name)


app = FastAPI(
    title="TicketFlow Evaluation & Tracing Service",
    description="Records agent-run traces (from the Agent Gateway) and exposes cost/latency/eval stats.",
    version="1.0.0",
    lifespan=lifespan,
)
