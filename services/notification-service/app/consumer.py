...
_HANDLED_EVENT_TYPES = {"BOOKING_CONFIRMED", "BOOKING_FAILED", "BOOKING_CANCELLED"}
    ...
    if event_type in _HANDLED_EVENT_TYPES:
        db.add(
            Notification(
                ...
                message=f"Booking {payload['booking_id']} is now {event_type}",
            )
        )
        log.info("notification_sent", booking_id=payload["booking_id"], event_type=event_type)
    ...
    while True:
        ...
        try:
            ...
            if attempt >= settings.kafka_max_retries:
                ...
                log.error("notification_event_dlq", event_id=envelope.get("event_id"))
                return
...
async def consume_loop() -> None:
    ...
    while True:
        ...
        try:
            ...
            log.error("notification_consumer_error", error=str(exc))
