...
from app.routes import mock, plan
...
async def lifespan(app: FastAPI):
    log.info("travel_planner_agent_startup", service=settings.service_name)
    await init_db()
    log.info("database_initialised")
    yield
    log.info("travel_planner_agent_shutdown", service=settings.service_name)


app = FastAPI(
    title="TicketFlow Travel Planner Agent",
    description="Multi-turn constraint gathering and template-based itinerary generation.",
    version="1.0.0",
    lifespan=lifespan,
)
